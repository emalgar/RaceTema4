...
#Transformamos los datos en tibble

cran <- as_tibble(mydf)

#Eliminamos el data frame original
...
#Visulizamos los datos directamente

cran

print("Escribiendo directamente el nombre del dataframe nos sale una tabla con los datos organizados en columnas")

#Utilizamos la función glimpse para la visualización

glimpse(cran)

print("Utilizando la función glimpse se muestra la información del dataframe organizada en filas")
...
pack_sum <- by_package %>% summarise(count = n(), unique = n_distinct(ip_id),countries = n_distinct(country),
    avg_bytes = mean(size, na.rm = TRUE), med_bytes = stats::median(size, na.rm = TRUE))
...
print(cran %>% filter(size_mb >= 0.5) %>% arrange(desc(size_mb)))
...
students2_tidy <- students %>% pivot_longer(cols = starts_with("male") | starts_with("female"), names_to = "sexo_clase", values_to = "count") %>% separate(col = sexo_clase, into = c("sex", "class"), sep = "_")
